...
class Greenhouse:
    def __init__(self, plants=[], N=50, M=50, step=1, drainage_rate=0.4, irr_threshold=5, plant_types=[],
        ...
        # list of plant types the greenhouse will support
    ...
    def add_plant(self, plant):
        if (plant.row, plant.col) in self.plant_locations:
            print("[Warning] A plant already exists in position ({plant.row, plant.col}). The new one was not planted.")
        else:
        ...
        if next_growth_index_plus_1 > plant.growth_index:
            for i in range(plant.growth_index + 1, next_growth_index_plus_1):
                ...
                for p in points:
                    ...
                    if point[0] >= 0 and point[0] < self.grid.shape[0] and point[1] >= 0 and point[1] < self.grid.shape[
                        1]:
                        plant.num_grid_points += 1
                        self.grid[point]['nearby'].add(plant.id)
    ...
    def show_animation(self):
        if self.animate:
            ...
            print(
                "[greenhouse] No animation to show. Set animate=True when initializing to allow animating history of greenhouse!")
